...
!write(*,*) ra,rb
  ...
  subroutine compute_frechet_to_crs(recs, vmod, jac_out)

    ! Compute Frechet matrix and store in CRS format in memory
    ! Two-pass approach: count non-zeros, then fill arrays
    ! Reuses existing calc_frechet_row for actual computation
    ...
    type(receivers), intent(in) :: recs
    type(scalar_field_2d), intent(in) :: vmod
    type(crs_matrix) :: jac_out

    ! local variables
    type(frechet_matrix_header) :: freha
    type(frechet_matrix_row) :: frero
    integer :: i, j, k, nrows, nnz, icol, irow

    ! Setup header to get n2 (number of model parameters)
    call setup_frechet_header(freha, vmod)
    ...
    ! Pass 1: Count rows and non-zeros
    nrows = 0
    nnz = 0
    do i = 1, recs%nn
       do j = 1, recs%nar(i)
          ...
          call calc_frechet_row(vmod, recs%ariv(i,j)%path, freha, frero)
          do k = 1, freha%n2
             if (frero%nze(k) == 1) nnz = nnz + 1
          end do
       end do
    end do

    ! Deallocate existing jac if allocated
    if (associated(jac_out%row)) deallocate(jac_out%row)
    if (associated(jac_out%col)) deallocate(jac_out%col)
    if (associated(jac_out%val)) deallocate(jac_out%val)

    ! Allocate CRS arrays
    jac_out%n1 = nrows
    jac_out%n2 = freha%n2
    allocate(jac_out%row(nrows + 1))
    allocate(jac_out%col(nnz))
    allocate(jac_out%val(nnz))

    ! Pass 2: Fill CRS arrays
    icol = 1
    irow = 0
    do i = 1, recs%nn
       do j = 1, recs%nar(i)
          irow = irow + 1
          jac_out%row(irow) = icol
          frero%val = 0.0_dbl
          frero%nze = 0
          call calc_frechet_row(vmod, recs%ariv(i,j)%path, freha, frero)
          do k = 1, freha%n2
             if (frero%nze(k) == 1) then
                jac_out%col(icol) = k
                jac_out%val(icol) = frero%val(k)
                icol = icol + 1
             end if
          end do
       end do
    end do
    jac_out%row(nrows + 1) = icol

    deallocate(frero%val, frero%nze)

  end subroutine compute_frechet_to_crs
